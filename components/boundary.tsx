      ...
      {label ? (
        <div
          className={clsx('absolute flex -translate-y-1/2 gap-x-1 text-[9px]', {
            'right-4 lg:right-4': size === 'small',
            'right-4 lg:right-9': size === 'medium',
          })}
        >
...
const Label = ({
  ...
  return (
    <div
      className={clsx(
        'px-1.5  leading-4 font-medium tracking-widest uppercase ring-5 rounded ring-gray-400 dark:ring-gray-950',
        {
          'bg-gray-800 text-gray-500': color === 'gray',
          'bg-pink-600 text-pink-200': color === 'pink',
          'bg-blue-600 text-blue-200': color === 'blue',
          'bg-cyan-200 dark:bg-cyan-600 text-black': color === 'cyan',
